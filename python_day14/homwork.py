...
import python_day13.DBUtils as db  # 引入数据库连接的模板
...
def sign():  # 定义一个登录的方法
    ...
    if 'password' not in data:  # key键的password不能为空
        ...
        count1 = db1.find_count("select * from tb_user where name=%s", (name,))
        count2 = db1.find_count("select * from tb_user where passwd=%s", (password,))
        count3 = db1.find_count("select * from tb_user where name=%s and passwd=%s", (name, password))
        if len(name) == 0:
